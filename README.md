...
Or manually: **Settings** → **Add-ons** → **Add-on Store** → **⋮** → **Repositories** → Add `https://github.com/sproft/hass-claude`

Then install **Claude Code** from the add-on store and start it.
...
- The add-on getting **`Killed` repeatedly on startup** on Proxmox / small-VM setups (community thread posts 74, 75, 77)
...
1. **Strips the repo down to the Claude Code add-on only** — the original `auto-monocle` Alexa camera bridge and the standalone `playwright-browser` add-on are not included here. They are great in their own right, but they are unrelated to running Claude Code in Home Assistant, and removing them shrinks the maintenance surface and the install footprint.
